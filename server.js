...
class Entity {
  ...
  upgrade(number) {
    if (
      ...
      if (this.labelswitch != null) {
        this.sendMessage("Press Shift key to switch class");
      }
      let ID = this.id;
      ...
      if (this.accel.x == null || this.velocity.x == null) {
       ...
       let slowdown = 1
        ...
        this.y += this.stepRemaining * this.velocity.y * slowdown / roomSpeed;       
              if (slowdown == -0.999) {this.frozen == false}
    }
  ...
  contemplationOfMortality() {
    ...
    if (this.isDead()) {
      ...
      let killText = notJustFood ? "" : "You have been destroyed by ",
      ...
      if (this.id === room.topPlayerID) {
        ...
        if (notJustFood) {
          ...
          usurptText += " has died by an unknown power.";
        }
    ...
    connect: (() => {
          ...
          case "0":
            ...
            function classChange() {
              switch (player.body.labelswitch) {
                ...
                case "Ac15":
                  player.body.define(Class.akafugi15);
                case "Sta15":
                  player.body.define(Class.stabilizer15);
                  break;
                case "De15":
                  player.body.define(Class.desperado15);
                case "As15":
                  player.body.define(Class.assasbrid15);
                   case "Ts15":
                  player.body.define(Class.twinseer15);
                   case "Oc15":
                  player.body.define(Class.ocelot15);
                   case "Ox15":
                  player.body.define(Class.oxygenator15);
                  case "Co15":
                  player.body.define(Class.compulsory15);
                  break;
              }
            }
                ...
                if (switcheroo != true) {
                  if (player.body.labelswitch == trueLabel + totalFrames) {
                    ...
                    for (let i = 0; i <= totalFrames; i++) {
                      setTimeout(() => {
                        player.body.define(
                          Class[definition + (totalFrames - (i + (i == 0)))]
                        );
                      }, speed * i);
                    }
          ...
          case "ChangeTankOne":
            {
              ...
              MakeAnimations("Sta", 40, "stabilizer", 100);
              MakeAnimations("As", 15, "assasbrid", 100);
              MakeAnimations("De", 15, "desperado", 100);
              MakeAnimations("Ts", 15, "twinseer", 100);
              MakeAnimations("Oc", 15, "ocelot", 100);
              MakeAnimations("Ox", 15, "oxygenator", 100);
              MakeAnimations("Co", 15, "compulsory", 100);
              //switcheroo code:
              classChange();
      ...
      const spawn = (() => {
        ...
        return (socket, name) => {
          ...
          // Dev hax
          if (socket.key === "test060606" || socket.key === "test060606") {
            body.name = "\u200b" + body.name;
          ...
          body.sendMessage("You have spawned! Welcome to my game. Have fun.");
          body.sendMessage("You will be invulnerable until you move or shoot.");
          body.sendMessage("Join my discord: https://discord.gg/BSxrPJs");
          ...
          if (doDamage) {
            ...
            if (!bail) {
              ...
              reductionFactor = Math.min(deathFactor._me, deathFactor._n);
                          ...
                          if(my.health <= 0 && my.body.labelswitch == 'DOM'){
                          }
                        }
                      /*************   POISON  ***********/
                      if (n.poison) {
...
function poison(element) {
  	entities.forEach(function(element) {
   	 ...
   	 if (element.poisoned) {// && element.type == 'tank'
        	...
        	if (element.poisonimmune == 0){element.poisoned = false}else{
       	var poisondeath = 1
        	...
        	if (!element.invuln && element.health.amount > element.health.max / (55 - element.poisonLevel)) {
          	element.health.amount -= (element.health.max / (55 - element.poisonLevel))*element.poisonimmune
          	element.shield.amount -= (element.shield.max / (35 - element.poisonLevel))*element.poisonimmune
        	}
}
...
var maintainloop = (() => {
  ...
  let spawnBosses = (() => {
      ...
      return {
        prepareToSpawn: (
          ...
          if (n === 1) {
            ...
            arrival = names[0] + " has come.";
          } else {
            begin = "Bosses are coming.";
            arrival = "";
            for (let i = 0; i < n - 2; i++) arrival += names[i] + ", ";
            arrival += names[n - 2] + " and " + names[n - 1] + " have come.";
          }
        },
        spawn: () => {
    ...
    return census => {
      if (timer > 200000 && ran.dice(16000 - timer)) {
  ...
  let spawnCrasher = census => {
    if (
      ...
      let type = ran.dice(80) ? ran.choose([Class.sentryyGun]) : Class.crasher;
      let o = new Entity(spot);
      o.define(type);
      o.team = -100;
    }
    ...
    if (msg.content.startsWith(prefix + "kill ")) {
      if (msg.author.id == owner_id) {
        ...
        if (sendError) {
          bot.createMessage(
            msg.channel.id,
            "Was unable to find an entity by the id: " + lookfor
          );
        }
      } else {
